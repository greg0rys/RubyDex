...
9 files inspected, 26 offenses detected:
...
### test.rb - (5 offenses)
  ...
  * **Line # 4 - convention:** Style/RedundantInterpolation: Prefer `to_s` over string interpolation.

    ```rb
    filename = "#{d.next_month.strftime("%d-%m-%Y-%H:%M")}"
    ```

  * **Line # 4 - convention:** Style/StringLiteralsInInterpolation: Prefer single-quoted strings inside interpolations.

    ```rb
    filename = "#{d.next_month.strftime("%d-%m-%Y-%H:%M")}"
    ```

  * **Line # 6 - convention:** Layout/TrailingEmptyLines: 1 trailing blank lines detected.
